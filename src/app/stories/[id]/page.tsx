...
export default async function StoryPage({params}:{params:Promise<{id:string}>}) {
  ...
  const story = db ? (await db.select().from(stories).where(eq(stories.id,id)).limit(1))[0] : demoStories.find(s=>s.id===id);
  if(!story) notFound();
  const refs = db ? await db.select().from(sources).where(eq(sources.storyId,id)) : [];
