...
const segments = [
  ...
  },
  {
    ...
    narration: paragraphs([
      ...
      "The editorial promise is simple: the machine can search, sort, reject, merge, and draft, but the user should receive a story. A completed result should already know why the evidence matters, what the human tension is, and where the unresolved risk remains. It should not ask the reader to do the research operation again. It should present the best supported version of the story, with citations close enough to every major claim that trust can be checked without breaking the narrative.",
      "That is why the final screen has to be judged by the manuscript, not by the amount of research debris behind it. The archive work is valuable only when it has been shaped into a readable account. If the story is not there, the product has not finished. If the story is there, the links become evidence instead of homework. This proof page exists to keep that distinction enforceable in code. The standard is the story, not the scaffolding, every single time for every completed result.",
    ]),
  },
];
